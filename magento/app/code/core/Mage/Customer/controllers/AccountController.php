...
/*         $this->getResponse()->setHeader('Login-Required', 'true');
        ...
        $this->_initLayoutMessages('catalog/session');
        $this->renderLayout(); */
	include_once('J:\\wamp\\www\\magento\\lib\\CAS.php');
	phpCAS::client(CAS_VERSION_2_0, 'localhost', 8443, 'cas');
	phpCAS::setNoCasServerValidation();
	phpCAS::forceAuthentication();
	$this->CASloginPostAction();
	// force CAS authentication
    }
/*
*POST CAS LOGIN 
*/
public function CASloginPostAction()
{
        $session = $this->_getSession();
		$username = phpCAS::getUser();
		if (!empty($username)) {
			try {
				$session->CASlogin($username);
    ...
    public function logoutAction()
    {
		...
		phpCAS::logoutWithUrl('localhost/magento');
